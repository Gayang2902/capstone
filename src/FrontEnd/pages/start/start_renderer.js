...
// 열기 버튼 클릭 시 Enter Master 모달 오픈 + 포커스 이동
function showEnterModal() {
    const modal = document.getElementById('enterMasterModal');
    modal.style.display = 'flex';
    const input = document.getElementById('enter-master-input');
    input.value = '';
    // 모달이 렌더링된 직후에 포커스
    setTimeout(() => input.focus(), 0);
}

// 새 파일 버튼 클릭 시 Create Master 모달 오픈 + 포커스 이동
function showCreateModal() {
    document.getElementById('enter-master-input').focus();
    const modal = document.getElementById('createMasterModal');
    modal.style.display = 'flex';
    const input = document.getElementById('create-master-input');
    input.value = '';
    setTimeout(() => input.focus(), 0);
}

// 이벤트 바인딩
document.getElementById('open-password-file')
    .addEventListener('click', showEnterModal);

document.getElementById('create-password-file')
    .addEventListener('click', showCreateModal);

document.getElementById('enter-master-cancel')
    .addEventListener('click', () => {
        document.getElementById('enterMasterModal').style.display = 'none';
    });

document.getElementById('create-master-cancel')
    .addEventListener('click', () => {
        document.getElementById('createMasterModal').style.display = 'none';
    });

// (필요 시) Connect 버튼 클릭 로직
